...
class CheckoutSummaryHandler(BaseHandler):
    ...
    async def handle(
        ...
        if msg_lower in ["confirm", "confirmar", "sim", "s", "ok", "1"]:
            ...
            # Garantir que o cliente existe no banco (Order.customer_id é NOT NULL)
    ...
    async def _ensure_customer_for_order(
        ...
        """Garante que o cliente existe no banco e que customer_context.customer_id está definido."""
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if not customer:
                    # Nome: priorizar contexto da conversa (collected_data) e depois customer_context
                    name = (
                    ...
                    # Cliente já existe: atualizar nome se tivermos um nome melhor no contexto
                    name_from_ctx = (
                        conversation_context.collected_data.get("name")
                        or customer_context.name
                    )
            ...
            logger.error(f"Erro ao garantir cliente: {e}", exc_info=True)
            return customer_context
    ...
    async def _create_order(
        ...
        try:
            ...
            # Total do pedido = soma dos subtotais dos itens (evita divergência)
            ...
            async with AsyncSessionLocal() as db:
                ...
                logger.info(f"Pedido criado: #{order.order_number} - Total: {order.total_amount}")

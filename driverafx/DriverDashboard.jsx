...
const DriverDashboard = () => {
  ...
  const loadDriverData = useCallback(async () => {
    try {
      ...
      const driverRes = await fetch(`${API_URL}/api/drivers/me`, {
      ...
      const activeRes = await fetch(`${API_URL}/api/drivers/me/deliveries/active`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const availableRes = await fetch(`${API_URL}/api/drivers/deliveries/available`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      // Buscar estatísticas
      const statsRes = await fetch(`${API_URL}/api/drivers/me/stats/today`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const statsData = await statsRes.json();
      setStats(statsData);

      // Buscar tempo trabalhado
      const timeRes = await fetch(`${API_URL}/api/drivers/me/time-worked`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const timeData = await timeRes.json();
      setTimeWorked(timeData);
  ...
  const updateDriverStatus = async (newStatus) => {
    try {
      const token = localStorage.getItem('token');
      await fetch(`${API_URL}/api/drivers/me/status`, {
        method: 'PUT',
        headers: {
  ...
  const acceptDelivery = async (deliveryId) => {
    try {
      const token = localStorage.getItem('token');
      await fetch(`${API_URL}/api/drivers/deliveries/${deliveryId}/accept`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const updateDeliveryStatus = async (deliveryId, newStatus) => {
    try {
      const token = localStorage.getItem('token');
      await fetch(`${API_URL}/api/drivers/deliveries/${deliveryId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
          ...
          >
            <div className="p-6">
              ...
              <div className="space-y-2">
                <button className="w-full flex items-center gap-3 px-4 py-3 bg-slate-700/50 hover:bg-slate-700 rounded-xl text-white transition-all duration-200">
...
// Modal de Detalhes da Entrega
const DeliveryDetailModal = ({ delivery, onClose, onUpdateStatus }) => {
  const statusInfo = DELIVERY_STATUS[delivery.status] || DELIVERY_STATUS.pending;
  const nextStatuses = {
    assigned: 'picked_up',
  ...
  const openMaps = () => {
    const address = encodeURIComponent(delivery.order?.endereco || '');
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-white font-semibold text-lg">
                #{delivery.order?.numero_pedido || delivery.id.slice(0, 8)}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-white">{delivery.order?.endereco || 'Não informado'}</p>
              {delivery.bairro && (

...
function Chats() {
  ...
  const loadChats = useCallback(async () => {
    logger.log('[Chats] Iniciando loadChats...')
    setLoadingChats(true)
    try {
      logger.log('[Chats] Chamando getChats()...')
      const data = await getChats()
      logger.log('[Chats] getChats() retornou:', data)
      setChats(data)
      logger.log('[Chats] Estado atualizado com', data.length, 'conversas')
    } catch (error) {
      logger.error('[Chats] Erro ao carregar chats:', error)
      setChats([])
    } finally {
  ...
  const handleNewMessage = useCallback((data) => {
    logger.log('Nova mensagem via WebSocket:', data)

    // Debounce: carregar conversas no máximo a cada DEBOUNCE_MS ms
    const now = Date.now()
    if (now - lastLoadTime > DEBOUNCE_MS) {
      logger.log('[Chats] Recarregando conversas (debounce ok)')
      setLastLoadTime(now)
      loadChats()
    } else {
      logger.log('[Chats] Pulando reload (debounce ativo)')
    }

...
export const useWebSocket = (url) => {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.current.onopen = () => {
        logger.log('WebSocket conectado:', wsUrl)
        setIsConnected(true)
  ...
  const send = useCallback((data) => {
    if (ws.current && ws.current.readyState === WebSocket.OPEN) {
      ...
      logger.warn('WebSocket não está conectado')
    }
  }, [])
...
export function useWebSocketEvent(event, callback) {
  useEffect(() => {
    ...
    import('../services/sharedWebSocket').then((module) => {
      ...
      logger.warn(`useWebSocketEvent: Evento '${event}' não pode ser registrado. Use useSharedWebSocketEvent.`)
    })

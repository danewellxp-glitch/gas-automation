...
class WebSocketService {
  ...
  connect() {
    ...
    logger.log('Conectando WebSocket:', url)
  ...
  tryReconnect() {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      logger.log('Maximo de tentativas de reconexao atingido')
      return
    }

    this.reconnectAttempts++
    logger.log(`Tentando reconectar (${this.reconnectAttempts}/${this.maxReconnectAttempts})...`)
  ...
  emit(event, data) {
    this.listeners.get(event)?.forEach((callback) => {
      try {
        ...
        logger.error('Erro no listener WebSocket:', e)
      }
    })
  }
}
